...
const rows = [];
for (const f of readdirSync(dir).filter(f => /^bt_crypto_.*.json$/.test(f))) for (const r of JSON.parse(readFileSync(`${dir}/${f}`, 'utf8'))) rows.push(r);
const short = (s) => s.replace('DELTAIN:', '').replace('USD.P', '');
