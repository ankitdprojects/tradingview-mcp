...
function Run($group, $symbols, $tfs, $extra) {
  $env:BT_SYMBOLS = $symbols; $env:BT_TFS = $tfs; $env:BT_EXTRA_INPUTS = $extra; $env:BT_OUT = "$base/bt_sma30L_$group.json"
  Write-Output "===== GROUP $group ====="
  node "$base/bt_sweep2.mjs" 2>&1
}
Run 'options' 'NSE:NIFTY260929C23500,NSE:NIFTY260929P23400,NSE:BANKNIFTY260929C56300,NSE:BANKNIFTY260929P56300,NSE:RELIANCE260929C1280,NSE:RELIANCE260929P1280,NSE:SBIN260929C1000,NSE:SBIN260929P1000,NSE:TATASTEEL260929C190,NSE:TATASTEEL260929P190' '5,15,30,60' "[$common,{`"id`":`"in_7`",`"value`":2},{`"id`":`"in_12`",`"value`":true}]"
