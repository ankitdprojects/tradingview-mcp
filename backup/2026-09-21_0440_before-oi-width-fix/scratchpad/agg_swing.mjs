...
const rows = [];
for (const f of readdirSync(dir).filter(f => /^bt_swing_.*.json$/.test(f))) for (const r of JSON.parse(readFileSync(`${dir}/${f}`, 'utf8'))) rows.push(r);
...
console.log('contract tf | OBOS-ON trades net win% PF DD% | OBOS-OFF trades net win% PF DD%');
const tot = {};
for (const k of Object.keys(byKey)) {
  const a = byKey[k]['OBOS on'] || {}, b = byKey[k]['OBOS off'] || {};
  ...
  for (const [lab, m] of [['OBOS on', a], ['OBOS off', b]]) {
...
for (const tf of ['D', 'W', 'all']) { const a = tot['OBOS on']?.[tf] || {}, b = tot['OBOS off']?.[tf] || {}; console.log(`${tf} | ${f0(a.net)} ${a.trades ?? 0} ${a.pos ?? 0}/${a.cells ?? 0} | ${f0(b.net)} ${b.trades ?? 0} ${b.pos ?? 0}/${b.cells ?? 0}`); }
const bySym = {};
for (const r of rows) { const s = bySym[r.symbol] ??= { 'OBOS on': 0, 'OBOS off': 0 }; if (r.net_profit != null) s[r.mode] += r.net_profit; }
console.log('\nTOTALS by contract | RULES | +STOP');
for (const s of Object.keys(bySym)) console.log(`${short(s)} | ${f0(bySym[s]['OBOS on'])} | ${f0(bySym[s]['OBOS off'])}`);
