...
const STRAT = { pineId: process.env.BT_STRAT_ID || "USER;7e62a80a99334d8ead50d5efa5230b96", pineVersion: process.env.BT_VER || "6.0", title: process.env.BT_STRAT_TITLE || "Pullback + Retest BT" };
const SCREENER = process.env.BT_RESTORE ? JSON.parse(process.env.BT_RESTORE) : { pineId: "USER;ad5ec9f07f2b4daeb989326e14cc13b8", pineVersion: process.env.BT_SCREENER_VER || "114.0", title: "Screener", inputs: [{id:"in_5",value:"Pullback + Retest"},{id:"in_43",value:false},{id:"in_32",value:false}] };
const REMOVE_NAME = process.env.BT_REMOVE_NAME || "Screener";
...
const scr = (await studies()).find(s => s.name === REMOVE_NAME);
...
const readReport = async () => {
  ...
  while (Date.now() - t0 < 150000) {
    ...
    const s = sig(m);
    if (!changed) { if (s !== prevSig) changed = true; else if (Date.now() - t0 > 100000) changed = true; else continue; }
...
for (const sym of SYMBOLS) {
  ...
  for (const tf of TFS) {
    ...
    await sleep(4000);
    // BT_LOAD_FROM (unix secs): page history back until the earliest loaded bar is older than this (or the feed runs out).
    if (process.env.BT_LOAD_FROM) {
      const lf = Number(process.env.BT_LOAD_FROM);
      for (let i = 0; i < 40; i++) {
        const st = await ev(`(function(){var ms=TradingViewApi.activeChart()._chartWidget.model().mainSeries();var b=ms.bars();var fv=b.valueAt(b.firstIndex());var more=true;try{more=ms.requestMoreDataAvailable()}catch(e){}return {first:fv&&fv[0],more:more,size:b.size()}})()`);
        if (!st || st.first == null || st.first <= lf || !st.more) { console.log(sym, tf, 'history loaded', st && st.size, 'bars from', st && new Date(st.first*1000).toISOString().slice(0,10)); break; }
        await ev(`(function(){try{TradingViewApi.activeChart()._chartWidget.model().mainSeries().requestMoreData(1000)}catch(e){}})()`);
        await sleep(2000);
      }
      await sleep(3000);
    }
...
await ev(`TradingViewApi.activeChart().getStudyById('${scrId}').setInputValues(${JSON.stringify(SCREENER.inputs || [])}); true`);
